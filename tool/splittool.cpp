...
void SplitTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
    ...
    FuncArea *funcArea = static_cast<FuncArea*>(poly);
    if(event->button() == Qt::LeftButton) {
        ...
        if(m_isCreating){
            Split split;
            QVector<QPolygon> polygons = split.split(funcArea->outline(), m_splitLine->line());
            if(polygons.isEmpty()){

            }else{
                delete funcArea; //delete the old one
                foreach(QPolygon poly, polygons){
                    funcArea = new FuncArea("未命名", poly);
                    funcArea->computeArea();
                    funcArea->computeCenter();
                    funcArea->setParentEntity(scene->currentFloor());
                }
            }
        }
